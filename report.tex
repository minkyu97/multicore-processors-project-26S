...
\title{\vspace{-2em}A Concurrent Open-Addressing Hash Table:\\
...
other and against two widely used sequential baselines,
\texttt{std::unordered\_map} and Google's \texttt{absl::flat\_hash\_map}.
...
implementations---\texttt{std::unordered\_map} and
\texttt{absl::flat\_hash\_map}---are fundamentally single-threaded
...
against the two sequential baselines that a practitioner would
otherwise reach for.
...
Each slot key occupies one of three logical states:
\texttt{EMPTY} (sentinel $-1$), \texttt{DELETED} (sentinel $-2$), or a
live user key. An insert may take a slot from
\texttt{EMPTY}$\rightarrow$live or
\texttt{DELETED}$\rightarrow$live; a delete transitions
live$\rightarrow$\texttt{DELETED} (tombstone). Lookups treat
\texttt{EMPTY} as a terminator (the probe sequence stops) but skip
over \texttt{DELETED} slots. Tombstoning is required because
eliminating a slot outright would break probe chains for other
keys that collided through it.
...
dispatches an \texttt{\#pragma omp parallel for schedule(static)}
across the user-provided arrays. Static scheduling was deliberate:
...
for \texttt{omp\_lock\_t}. Absl baselines link against
\texttt{absl}~20240116. The build is orchestrated by CMake; the
repository contains \texttt{CMakeLists.txt} and \texttt{scripts/}
with all driver scripts.

\subsection{Workload generator}
\texttt{bench\_hashmap} accepts seven arguments: \emph{table\_size,
num\_ops, sequential|parallel, threads, probing, key\_dist, reps}.
...
\end{itemize}
Each key $k$ is associated with value $10k$; \texttt{get\_batch}
...
min, and max. After the last rep, every inserted key is re-queried
and any mismatch fails the run. This protocol is implemented in
\texttt{benchmark\_table} in \texttt{src/bench\_hashmap.cpp}.

\subsection{Baselines}
\texttt{bench\_map\_baselines} wraps \texttt{std::unordered\_map} and
\texttt{absl::flat\_hash\_map} in the same harness. Both are
sequential single-threaded baselines; we run them to contextualize
our parallel numbers against production-grade C++ hash tables.

\subsection{Sweeps}
The \texttt{scripts/} directory contains four drivers, each of which
fixes three axes and sweeps the fourth:
\begin{itemize}
...
correctness (zero mismatches in the verification pass) and (ii)
